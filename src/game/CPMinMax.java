 ...
 */
public class CPMinMax extends ComputerPlayer {
     ...
     */
    public CPMinMax(float notAllowedHue, int noImageID, String name, int maxDepth) {
    ...
    private int GetNextMove(Player[][] boardPlayer, Player realOpponent, boolean isMaximizing, int currentDepth, Integer[] rowColumnIndexes)
    {
        ...
        else
        {
            ...
            for(int[] nextMove : possibleMoves)
            {
                ...
                int currentBoardValue = GetNextMove(nextBoard, realOpponent, nextIsMaximizing, currentDepth + 1, new Integer[] {0, 0}); // the Integer is not important..
